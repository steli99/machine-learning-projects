...
Usage: python scripts/execute_notebook.py notebooks/Linear_Regression.ipynb
Outputs (including figures) are embedded in the notebook. Network data are required.
"""
import os
...
# Preserve measured scalar / small-array metrics for the accompanying report.
import numpy as np
metrics={}
for key,value in namespace.items():
    if any(token in key.lower() for token in ['error','cod_','alpha_opt','lambda_opt','ci_w','best_features_subset','best_score','test_err','train_err','normalized_error','raw_error','comparison','zero_in_ci']):
        if isinstance(value,(int,float,str,np.number)):metrics[key]=value.item() if isinstance(value,np.number) else value
        elif isinstance(value,np.ndarray) and value.size<100:metrics[key]=value.tolist()
        elif isinstance(value,(list,tuple)) and len(value)<100 and all(isinstance(v,(int,float,np.number)) for v in value):metrics[key]=list(map(float,value))
        elif hasattr(value,'to_dict'):metrics[key]=value.to_dict()
result['metrics']=metrics
report=path.parent.parent/'docs'/f'{path.stem}_execution.json'
